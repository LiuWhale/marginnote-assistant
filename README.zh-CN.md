...
当前公开的 0.4.x 仍是预览版，不能被误认为终局。现在的聊天面板、回答下方按钮、设置页、队列、日志和第一阶段 Agent Workspace 都只是迁移脚手架。长期目标是 **MarginNote Knowledge Agent OS**：一个 Notebook Knowledge IDE，主语不是 prompt 或回答，而是真实 MarginNote note、脑图节点、摘录、卡片、文档、复习任务、workflow、技能、外部自动化请求和操作证据。终局必须支持 `zero-message workflow`：用户不输入任何 prompt，只打开 notebook，就能看到对象状态、材料覆盖、脑图缺口、卡片缺口、最近失败事务、可执行 workflow 和待确认写入。

路线图必须分成四层。`v0.4.x` 是 Chat Companion；`v1.x` 应该成为稳定 Study Copilot，体验对标 MarginNote 自带 AI；`v2.x` 必须成为 Native Knowledge Editor，能读取和编辑现有 `noteId` 对象，并通过 Diff、验证和回滚闭环；`v3.x` 才是 Notebook Knowledge OS，默认打开 `Notebook Workspace`，而不是空聊天框。`Chat Mode` 仍然用于快速阅读问答和输入意图，但会降级成 Object Browser、Mindmap Studio、Card Factory、Knowledge Graph、Workflow Builder、External Automation Gateway、Skill Center 和 Operation Ledger Explorer 旁边的命令面板。每次 AI 写入都应进入带验证和回滚证据的 Operation Ledger；跨 notebook 知识层、工作流运行时、外部 URL/API 自动化和可分享技能包都应成为一等产品界面。终局的七个不可替代内核是 Live MN Object Kernel、Source Registry、Operation Compiler、Transactional Native Editor、Workflow Runtime、Skill Runtime 和 Verification Agent。换句话说，如果一个所谓终局仍然只是“发送、生成脑图、生成卡片、设置和日志更好用”，它仍然不是 v3。
...
- 默认进入第一阶段 `Notebook Workspace` 总览，而不是空聊天页。总览会聚合当前焦点对象、对象数量、脑图树缓存、复习队列、workflow run 和 Operation Ledger 计数，并提供扫描 MN 对象、读取当前脑图树、生成操作计划、查看复习卡、工作流和账本证据的直接动作。
...
- `Notebook Workspace` 现在带第一阶段 `Notebook Runbook`：它把当前 notebook 的上下文、MN 原生对象扫描、脑图基线、操作计划、workflow runtime 和账本证据排成可执行检查清单，每一步都有状态、证据行和对应动作按钮，并在顶部提供 `继续下一步` 和 `自动准备`。`自动准备` 会把安全预检动作顺序跑完，例如扫描 MN 对象、读取脑图基线、生成操作计划，但不会直接写入 MarginNote；每次运行都会形成 `notebook_runbook_preflight` 账本证据，并在 Runbook 区显示最近状态。
- 对话不再作为工作台里的同级 tab，而是独立 `Command Pane`。Workspace 模式保留输入框和发送按钮，但默认收起对话历史，把 Notebook Workspace、对象、操作、知识和工作流作为主工作区；需要连续问答时切到 Chat Mode 展开完整对话。
...
- 第一阶段 Verification Agent 会在操作证据链里输出 `codex.mn.verificationReport.v1`。状态只有 `PASS`、`FAIL`、`UNKNOWN`；缺少原生对象 probe 时会保持 `UNKNOWN`，不会把日志成功当成真实通过。
