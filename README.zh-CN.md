...
- `Notebook Workspace` 现在带第一阶段 `Notebook Runbook`：它把当前 notebook 的上下文、MN 原生对象扫描、脑图基线、操作计划、workflow runtime 和账本证据排成可执行检查清单，每一步都有状态、证据行和对应动作按钮，并在顶部提供 `继续下一步` 和 `自动准备`。`自动准备` 会把安全预检动作顺序跑完，例如扫描 MN 对象、读取脑图基线、生成操作计划，但不会直接写入 MarginNote。
