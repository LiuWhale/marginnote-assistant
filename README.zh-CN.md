...
当前公开的 0.4.x 仍是预览版。长期目标是 **MarginNote Knowledge Agent OS**，不是聊天框加更多按钮，而是一个双模式产品：`Chat Mode` 对标 MarginNote 自带 AI 的轻量阅读对话；`Agent Workspace Mode` 则把 PDF 选区、卡片、脑图节点、文档、notebook 和外部文件变成可寻址对象。每次 AI 写入都应进入带验证和回滚证据的 Operation Ledger；跨 notebook 知识层、工作流运行时、外部 URL/API 自动化和可分享技能包都应成为一等产品界面。终极版里，聊天是入口，不是终局；Agent Workspace 才是生产系统，产品中心必须是对象、操作和证据。
...
- `Agent Workspace` 里显示第一阶段 Operation Compiler。`agent_plan` 会返回结构化 `operationPlan`、`verificationPlan` 和编译检查，写入类动作会先显示计划步骤、写入数量、原生能力 dry-run 结果、确认要求和写后验证义务，再被当成真实 MarginNote 编辑。
