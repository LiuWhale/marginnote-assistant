...
当前公开的 0.4.x 仍是预览版。长期目标是 **MarginNote Knowledge Agent OS**，不是聊天框加更多按钮。终极版默认首屏应该是 `Notebook Workspace`：围绕真实 MarginNote 对象、脑图 Diff、卡片覆盖率、复习队列、workflow 和操作证据工作的知识编辑界面。`Chat Mode` 仍然用于快速阅读问答和输入意图，但不是终局产品中心。每次 AI 写入都应进入带验证和回滚证据的 Operation Ledger；跨 notebook 知识层、工作流运行时、外部 URL/API 自动化和可分享技能包都应成为一等产品界面。
