...
- `Agent Workspace` 里显示第一阶段 Operation Compiler。`agent_plan` 会返回结构化 `operationPlan`、`verificationPlan` 和编译检查，写入类动作会先显示计划步骤、写入数量、dry-run/确认要求和写后验证义务，再被当成真实 MarginNote 编辑。
