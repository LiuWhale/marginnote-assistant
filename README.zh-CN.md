...
- `Notebook Workspace` 现在带第一阶段 `Notebook Runbook`：它把当前 notebook 的上下文、MN 原生对象扫描、脑图基线、操作计划、workflow runtime 和账本证据排成可执行检查清单，每一步都有状态、证据行和对应动作按钮，并在顶部提供一个 `继续下一步` 按钮，自动推进第一个真正需要处理的缺口。
...
```bash
python3 package_release.py 0.4.34
```
...
```bash
python3 release_acceptance.py release/CodexCompanion-0.4.34-latest-dist.zip --json
```
