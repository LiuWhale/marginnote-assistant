...
本文件定义的是当前公开预览版和 v1.x 的发布级可用目标，不定义 v3.0 终局。终极形态另见 `docs/ULTIMATE_PLUGIN_DESIGN.md`，它的边界不是“能聊天、能生成脑图”，也不是当前聊天插件的增强版；终极目标是 **MarginNote Knowledge Agent OS**。终局路线明确采用 **知识编辑工作台 + 自动学习代理**：默认首屏应是 `Notebook Workspace`，显示当前 notebook 的对象、脑图、卡片、知识关系、workflow、技能、外部自动化请求、复习状态和操作账本；`Chat Mode` 只是快速阅读和意图输入入口，不能成为高阶知识工作的中心。
