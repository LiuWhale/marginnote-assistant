...
1. 插件 UI
   ...
   - 首屏先提供 `Chat Mode / Agent Workspace` 双模式切换；`Chat Mode` 展开完整 `Command Pane` 对话输入和回答流，`Agent Workspace` 只保留可折叠 Command Pane 的输入入口，默认展开 `Notebook Workspace`、对象、操作、知识、工作流工作区，默认入口是对象区的 `Knowledge Console`，不是聊天消息流；`Agent Workspace` 顶部必须有 `Workspace Navigator`，把 `Knowledge Console`、`Mindmap Studio`、`Card Factory`、`Operation Ledger`、`Knowledge Graph`、`Workflow Builder` 和 `Skill Center` 作为一等入口，而不是让用户只理解底层 tab；`Notebook Workspace` 必须显示 `Source Registry`、`Study Program` 和 `Notebook Runbook`，先让用户看到当前 MN 文档、PDF 缓存、上传文件和搜索根是否真实可读，再在用户未输入 prompt 时给出覆盖率、缺口和推荐 workflow，并把当前 notebook 的上下文、MN 对象扫描、脑图基线、操作计划、workflow 和账本证据排成可执行状态清单；Runbook 必须提供 `继续下一步` 和安全 `autoPlan/自动准备` 入口推进真实缺口；自动准备必须留下 no-write preflight run 证据，并能从 Operation Ledger 打开；设置和历史作为独立页面打开，文件路径管理、AI 后端、诊断和低频配置不占用主工作区。
