...
当前 0.4.x 只能称为 Operation Compiler 第一阶段：它已经把写入类意图拆成结构化计划，并把计划、写入数量和验证要求暴露给前端和 doctor/单文档验收清单；但它还没有完成逐节点 native dry-run、跨对象残留扫描、完整回滚证明和技能包级 schema 迁移。因此它是从“聊天按钮”走向“对象操作系统”的关键断层，不是终局完成证明。
