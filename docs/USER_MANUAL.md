...
当前公开版本是 **0.4.55**：https://github.com/LiuWhale/marginnote-assistant/releases/tag/v0.4.55。它不是最终 v1.0；基础问答、草稿制卡、草稿脑图、目标任务、队列、文件上传、PDF 缓存、标注 PDF 副本导出、运行态诊断和双语 README 已经具备，可靠的 MarginNote 原生可见高亮、签名安装包、公证安装包和跨机器验收仍在补证据。
...
| 历史页 | 当前打开文件的历史对话 | 回看或清空当前文件的会话；切换文件不会续接上一文件的对话 |
...
回答下方的 `生成脑图树` 使用另一条安全路径。插件会先读取完整 notebook 脑图树；缓存必须是刚更新、未截断的 `whole_notebook` 快照，局部子树、过期快照、未来时间戳或截断结果都会触发重新读取，不会继续生成。回答标题和内容至少要与某个已有节点形成三个互不重叠的语义信号，才允许自动挂接；一个中文短词产生的重叠字片不会被重复计数，一两个泛化词相同仍会回退到当前文档根。写入前原生插件还会重新核对整树指纹、父节点的 noteId、标题、用于路由的正文快照和文档身份；即使节点总数不变，只要生成期间发生改名或正文变化也会阻断。文档根只有在 Codex 标记和标题都精确匹配且结果唯一时才会复用；重复标记、扫描不完整或标题变化会阻断写入。回答级计划只创建新节点，不会重命名、移动、合并或覆盖已有节点。

每条成功完成的回答都会保存完整 Markdown，因此当前对话和历史页的 `复制` 内容一致。历史写入采用会话锁和原子替换，并发完成的回答不会互相覆盖。为避免长历史反复占用模型上下文，重新发送给模型的单条历史仍限制在 5,000 字符。错误、超时和系统状态只作为普通消息显示，不提供 `复制` 或 `生成脑图树`。

这条自动挂接路径只允许新增节点。与现有节点同名的生成节点会被去重，其未重复子节点会继续保留；插件不会修改已有节点标题、正文和父子关系。所有草稿写入都必须携带生成时的 `topicid` 和 `bookmd5`，并进入可拒绝的事务；缺少绑定、切换文档、跨文档父节点或去重扫描不完整都会在创建卡片/脑图前停止。拒绝只按本次事务记录的 `createdNoteIds` 删除，不调用全局 Undo，也不会把卡片分类 ID 当作额外删除授权。
...
一个脑图可以同时包含多篇 PDF 或其他来源。普通单文档对话仍以 MarginNote 阅读器当前打开的文件为锚点：切换文件后，插件会立即刷新标题、路径和文档身份，为新文件使用独立历史和独立 PDF 缓存。上一文件仍在生成的回答不会显示到新文件中，上一文件排队的动作也不会在新文件上执行。选中的脑图节点可以作为补充材料，但不能覆盖当前打开文件的身份。

需要一个回答同时比较或综合多个指定文件时，使用对话标题栏的 `资料 N`：

1. 点击 `资料 N` 进入独立资料页。勾选当前文件、当前 notebook 中可解析的其他文件或已上传文件；新的本地文件统一通过 `Add Files` 上传后选择。
...
一次批量操作允许部分失败。超过大小限制、读取失败或后端拒绝的文件会逐项列出原因，但已经成功的文件不会被撤销。成功文件会自动选中，随后自动保存并验证资料集。修复失败文件后再次点 `Add Files` 即可；重复批次会继续追加到同一显式资料集，因此总文件数超过 20 时应分批添加。上传期间切换对话或当前文档时，已成功文件仍保留，但不会被错误地自动加入新的上下文，需要在目标对话中重新选择。
...
多文件资料工作区必须使用 Codex CLI。后端为 `auto` 时必须找到可用 Codex CLI，并且不会回退到 OpenAI API；后端为 OpenAI API-only 时会在模型调用前阻止，因为远程 API 无法读取本地软链接。不要通过切换后端绕过资料验证，否则回答会缺少文件而不自知。

资料是读取范围，不是写入授权。即使一次读取了多个文件，生成卡片或脑图仍必须有一个明确的当前 MarginNote notebook 和一个经过验证的写入目标。插件不会因为选中多个资料而向多个 notebook 写入，也不会自动选择脑图。

故障恢复：

- `Operation not permitted` 或不可读：给 Companion/Codex 所在进程所需的 macOS 文件访问权限，保持文件可用，然后点 `验证资料`。
- 文件已移动、删除或软链接断开：恢复原路径后重新验证，或移除该资料。断开的链接会在模型调用前阻止，不能静默降级为部分读取。
- 文本提取失败：保留原文件可读性并重新验证；只有原文件和提取文本都不可读时才必须移除或修复。
- 回答缺少某个 source ID：把结果视为不完整；当前版本不会自动重跑，应先检查该资料状态再重新发送。
...
从 `0.4.54` 起，Codex CLI 没有固定生成超时：普通问答、全文精读、脑图和多文件资料任务都会持续运行，直到 Codex 返回或用户点击 `停止`。`停止` 会终止已注册的 Codex CLI 进程组；网络永久卡住时应主动停止，而不是等待插件在 90 秒后误判失败。
...
截至 2026-08-25，`v0.4.55` 是当前公开预览版。该版本保留完整多文件资料工作区和 Codex CLI 无固定超时行为，并修复同文档脑图快照被误判为不完整的首击生成失败；MN4 运行态仍需要重新打开面板或重启 MN4 后才会上报新版 WebView/native 能力事件。当前仍有这些发布阻塞：
...
这些限制不影响使用 `v0.4.55` zip 或 `.mnaddon` 作为公开预览版继续试用，但影响“发给别人当最终正式产品”的判断。
