...
| 位置 | 作用 | 典型使用 |
| --- | --- | --- |
| Notebook Workspace | Agent Workspace 首屏总览，聚合当前焦点对象、对象数量、脑图树缓存、复习队列、workflow run、Operation Ledger 计数、Study Program 和 Notebook Runbook | 先看 `Study Program` 的覆盖率、缺口和推荐 workflow；也可以点 `自动准备` 跑安全预检并留下 preflight 账本证据，或点 `继续下一步` 逐步扫描 MN 对象、读取当前脑图树、生成操作计划、检查 workflow 和账本证据 |
...
截至 2026-06-27，`v0.4.37` 是当前公开预览版发布候选；本轮目标是完成 GitHub Release、双语 README、release zip smoke、`.mnaddon` smoke、install dry-run 和本机 0.4.37 安装替换；MN4 运行态需要重新打开面板或重启 MN4 后才会上报新版 WebView/native 能力事件。当前仍有这些发布阻塞：
...
这些限制不影响使用 `v0.4.37` zip 或 `.mnaddon` 作为公开预览版继续试用，但影响“发给别人当最终正式产品”的判断。
