...
| 位置 | 作用 | 典型使用 |
| --- | --- | --- |
| Notebook Workspace | Agent Workspace 首屏总览，聚合当前焦点对象、对象数量、脑图树缓存、复习队列、workflow run、Operation Ledger 计数和 Notebook Runbook | 按 runbook 逐步扫描 MN 对象、读取当前脑图树、生成操作计划、检查 workflow 和账本证据 |
...
- `Notebook Workspace`：这是 Agent Workspace 的首屏工作台。上半部分显示当前焦点、对象数量、脑图基线、复习队列、workflow 和账本计数；下半部分的 `Notebook Runbook` 会把当前 notebook 的下一步排成检查清单。Runbook 现在包含确认上下文、扫描 MN 对象、读取脑图基线、生成操作计划、检查工作流、核对操作证据六步，每一步都有 `通过 / 需操作 / 阻断 / 等待` 状态、证据行和动作按钮。它的作用是让你先看到“这个 notebook 现在缺什么”，再进入对象、脑图、工作流或账本，而不是回到聊天消息里找按钮。
...
截至 2026-06-27，`v0.4.33` 是当前公开预览版发布候选；本轮目标是完成 GitHub Release、双语 README、release zip smoke、`.mnaddon` smoke、install dry-run 和本机 0.4.33 安装替换；MN4 运行态需要重新打开面板或重启 MN4 后才会上报新版 WebView/native 能力事件。当前仍有这些发布阻塞：
...
这些限制不影响使用 `v0.4.33` zip 或 `.mnaddon` 作为公开预览版继续试用，但影响“发给别人当最终正式产品”的判断。
