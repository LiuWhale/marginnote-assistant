...
更远的终极目标不是“聊天框加几个按钮”，而是 **MarginNote Knowledge Agent OS**。它必须是双模式产品：`Chat Mode` 保持像 MarginNote 自带 AI 一样的轻量阅读对话；`Agent Workspace Mode` 把 MarginNote 里的 PDF 选区、卡片、脑图节点、文档、notebook 和外部文件都当成可操作对象，把每次 AI 写入记录成可验证、可回滚的 Operation Ledger，并把跨 notebook 知识层、工作流运行时、外部 URL/API 自动化和可分享技能包做成一套生产系统。聊天是入口，不是终局；Agent Workspace 才是生产系统。只要高阶工作仍只能靠输入一句话再点回答下方按钮，就不算终极版。
...
| 位置 | 作用 | 典型使用 |
| --- | --- | --- |
| 对话页 / Chat Mode | 轻量阅读对话，固定输入框、发送按钮、顶部目标区、2x2 常用任务和常驻工具区 | 直接提问、执行目标、解释选区、制卡、脑图、补脑图、重组、查看进度 |
...
WebView 顶部先提供 `Chat Mode / Agent Workspace` 双模式切换。`Chat Mode` 只展开轻量对话、回答流、固定输入框和发送按钮，适合像 MarginNote 自带 AI 一样快速解释当前选区或连续追问；`Agent Workspace` 展开对象、操作、知识和工作流工作区，默认入口是对象区的 `Knowledge Console`，不是聊天消息流。切到 `Chat Mode` 会聚焦对话面板；切回 `Agent Workspace` 会恢复上一次非对话工作区。`Agent Workspace` 顶部还有 `Workspace Navigator`，可直接跳到 `Knowledge Console`、`Mindmap Studio`、`Card Factory`、`Operation Ledger`、`Knowledge Graph`、`Workflow Builder` 和 `Skill Center`，这些入口会选中对应工作区并滚动到目标模块。
...
- `操作`：集中显示目标脑图、当前脑图树缓存、Mindmap Studio、最新脑图 Diff 编辑台、Agent 操作计划、下一步动作、执行验证和事务中心。Mindmap Studio 不是回答下方按钮的别名，它是第一阶段脑图对象操作台：点击 `读取现有脑图` 可以刷新真实脑图树，点击 `预览 Diff` 会按当前回答或输入生成变更预览，点击 `应用所选` 会应用当前选中的 Diff，点击 `验证事务` 会刷新最近事务证据，点击 `回滚事务` 会请求 MN4 删除本次事务新增节点并返回残留结果。生成脑图后，即使聊天滚动到别处，最近一次 Diff 的新增/更新/合并/移动/建议删除统计仍会留在这里；接受或拒绝 AI 编辑后，最近事务的回滚状态、事务对象、创建 noteId 和残留 noteId 也会留在这里。局部脑图 Diff apply 也会进入事务中心，显示本次 transactionId、created/applied noteId、失败数和逐操作验证摘要；生成成功后先等待确认，你可以直接点 `保留`，也可以点 `回滚` 删除本次新增节点，或点 `验证/证据` 查看事务报告。如果 Diff 里包含删除建议，普通接受不会直接删除旧卡片；事务中心会另外显示 `删除 / 忽略`，需要你二次确认后才删除目标 noteId。
...
截至 2026-06-27，`v0.4.28` 是当前公开预览版发布候选；本轮目标是完成 GitHub Release、双语 README、release zip smoke、`.mnaddon` smoke、install dry-run 和本机 0.4.28 安装替换；MN4 运行态需要重新打开面板或重启 MN4 后才会上报新版 WebView/native 能力事件。当前仍有这些发布阻塞：
...
这些限制不影响使用 `v0.4.28` zip 或 `.mnaddon` 作为公开预览版继续试用，但影响“发给别人当最终正式产品”的判断。
