...
当前版本仍是 RC，不是最终公发版。基础问答、草稿制卡、草稿脑图、目标任务、队列、文件上传、PDF 缓存、标注 PDF 副本导出和诊断面板已经具备；可靠的 MarginNote 原生可见高亮、签名安装包、公证安装包和跨机器验收仍在补证据。
...
本文把“已经可用的功能”和“仍在 RC 验证的能力”分开写。手册里写“当前 RC 仍在验证”的地方，不应当当作最终发布承诺。
...
| 位置 | 作用 | 典型使用 |
| --- | --- | --- |
| 对话页 | 主工作区，固定输入框、发送按钮、顶部目标区、2x2 常用任务和常驻工具区 | 直接提问、执行目标、解释选区、制卡、脑图、补脑图、重组、查看进度 |
...
| 高亮原文 | 点原文工具里的高亮，必要时回到 PDF 重新选中文字 | 尝试 MN4 原生高亮；当前 RC 仍需实测证据 |
...
- 本地 Companion 服务：监听 `http://127.0.0.1:48761`，负责 AI 请求、队列、草稿、历史、文件、导出 PDF 副本、诊断和发布验收。
...
- `对话`：主工作区。这里固定保留输入框和发送按钮，也显示上下文、目标、草稿、进度和引导按钮。主操作区先放一次性目标，再放 2x2 常用任务：解释、制卡、脑图、精读；当前脑图工具和原文工具常驻显示，分别提供补到当前/重组，以及高亮/导出/状态。缺上下文时按钮短提示会显示需选区、需节点、需文档或需能力。运行态提示只保留刷新和去设置，低频维护动作放到设置页。
- `按钮`：管理预设模板和自定义按钮。预设可填入输入框，也可添加成自定义按钮；自定义按钮可直接置顶或移出主界面。
- `设置`：配置 AI 后端、模型、速度、权限、代理、OpenAI Key，并按 AI 与连接、权限与文件、MN 运行态、验收四组运行低频诊断。
- `文件`：上传补充文件或登记本地文件路径，让模型在回答中使用这些材料。
- `历史`：查看或清空当前 notebook/book 的历史对话。
...
当前脑图草稿主要用 Markdown 层级表达。`##` 通常表示一个主要分支，下面的条目会作为子节点或节点正文。
...
- MarginNote 原生高亮：主界面按钮是 `高亮下一选区`，点击后会让 MN4 插件进入一次性的下一次 PDF 选区自动高亮模式；如果当前已经有有效选区，则立即尝试调用官方 `highlightFromSelection()`。插件连接时会注册 PDF 选区弹出菜单 observer，并尝试在 PDF 选中文本菜单里追加 `Codex 高亮选区`；设置页 `高亮采证` 会启动同一路线，并把当前状态整理成 `waiting_selection`、`verifying`、`failed` 或 `complete`，同时显示原生高亮和 PDF 选区菜单两项是否通过。当前 RC 仍在补可靠可见高亮证据。
...
历史记录用于连续阅读同一份资料，不用于跨所有文档共享永久记忆。
...
当前 RC 的 MarginNote 原生可见高亮仍在验证。优先点设置页 `高亮采证`，然后回到 PDF 重新选中一小段文字；面板会显示原生高亮和选区菜单是否通过。如果只是想日常高亮，可以直接点原文区 `高亮下一选区`，再回到 PDF 选中文字；如果当前页面已有真实选区，也会立即尝试消费这次选区。也可以直接双击 `Collect Native Highlight Evidence.command` 让采证命令主动请求一次高亮，并在 90 秒内等待你重新选区后的 posted/failed 结果。如果失败，运行 `刷新MN能力` 和 `运行态采证`。需要可导出的结果时，优先使用 `导出标注 PDF` 生成副本。

### 安装包打不开或提示不受信任

当前 `.pkg` 仍是 unsigned / not notarized 的 RC 包，适合内部测试，不适合作为正式公发安装包。最终对外发布需要 Developer ID Installer 签名、Apple notarization 和跨机器安装证据。
...
截至 2026-06-12，当前版本仍有这些发布阻塞：

- MN4 运行态有时不会热重载最新插件，需要重新打开面板或重启 MN4。
- MarginNote 原生可见高亮仍缺活跃 PDF 选区下的完整证据。
- 当前 `.pkg` 未签名。
- 当前 `.pkg` 未 notarize。
- 还缺第二用户或第二机器的结构化安装验收。

这些限制不影响把它作为开发预览版继续试用，但影响“发给别人当正式产品”的判断。
