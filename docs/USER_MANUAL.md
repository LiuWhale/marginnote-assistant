...
一个脑图可以同时包含多篇 PDF 或其他来源，但对话上下文以 MarginNote 阅读器当前打开的文件为准。切换文件后，插件会立即刷新标题、路径和文档身份，为新文件使用独立历史和独立 PDF 缓存。上一文件仍在生成的回答不会显示到新文件中，上一文件排队的动作也不会在新文件上执行。选中的脑图节点可以作为补充材料，但不能覆盖当前打开文件的身份。需要综合多个文件时，应明确使用跨文档或 notebook 知识检索。
