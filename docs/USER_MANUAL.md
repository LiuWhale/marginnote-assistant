...
- 本地 Companion 服务：监听 `http://127.0.0.1:48761`，负责 AI 请求、队列、草稿、历史、文件、导出 PDF 副本、诊断日志和发布验收。0.4.26 起诊断日志逻辑拆到独立 `diagnostic_log.py` 模块，但面板和 API 行为不变。
...
当前 GitHub Release 提供的是 zip 安装包；`.pkg` 路线仍缺 Developer ID Installer 签名和 Apple notarization 证据，适合内部维护，不适合作为最终 v1.0 公发安装器。最终对外发布需要签名、公证和跨机器安装证据。
...
截至 2026-06-25，`v0.4.26` 已完成 GitHub Release、双语 README、release zip smoke、install dry-run 和本机 0.4.26 安装替换；MN4 运行态需要重新打开面板或重启 MN4 后才会上报新版 WebView/native 能力事件。当前仍有这些发布阻塞：

- MarginNote 原生可见高亮仍缺活跃 PDF 选区下的完整证据。
- `release_sha256_manifest` 仍会因缺少对应 `.pkg` entry 阻塞最终 release acceptance。
- 当前 `.pkg` 未签名、未 notarize。
- 还缺第二用户或第二机器的结构化安装验收。
- 还缺同一文档完整按钮/工作流验收 PASS evidence。

这些限制不影响使用 `v0.4.26` zip 作为公开预览版继续试用，但影响“发给别人当最终正式产品”的判断。
