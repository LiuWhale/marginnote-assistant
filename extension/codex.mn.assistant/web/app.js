(function() {
  ...
  var state = {
    ...
    queueGuardRestoreIdentityKey: '',
    queueGuardRestoredIdentityKey: '',
    queueRuntimeReady: false,
  ...
  var requiredControlIds = [
    ...
    'sourceWorkspaceDiagnosticsBody',
  ...
  function sourceUsageIds(sourceUsage, modernKey, legacyKey) {
    sourceUsage = sourceUsage || {};
    var values = sourceUsage[modernKey] || sourceUsage[legacyKey] || [];
    return Array.isArray(values) ? values.map(function(value) { return String(value || ''); }).filter(Boolean) : [];
  }

  function sourceUsageDisplayName(sourceId) {
    var candidate = sourceWorkspaceCandidateById(sourceId);
    if (candidate) return String(candidate.title || sourceId);
    var source = sourceWorkspaceRecordById(sourceId);
    return source ? String(source.displayName || source.title || sourceId) : String(sourceId || '未知资料');
  }

  function buildSourceUsageFooter(sourceUsage) {
    sourceUsage = sourceUsage || {};
    var readIds = sourceUsageIds(sourceUsage, 'readIds', 'read');
    var unreadIds = sourceUsageIds(sourceUsage, 'unreadIds', 'unread');
    var missingIds = sourceUsageIds(sourceUsage, 'missingIds', 'missing');
    var total = Number(sourceUsage.sourceCount || readIds.length + unreadIds.length + missingIds.length || sourceWorkspaceSelectionIds().length);
    if (!sourceUsage.active && !total) return null;
    var footer = document.createElement('div');
    footer.className = 'source-usage-footer ' + (sourceUsage.complete === false ? 'incomplete' : 'complete');
    var summary = document.createElement('div');
    summary.className = 'source-usage-summary';
    summary.textContent = '已读取 ' + readIds.length + '/' + total + ' 个文件';
    footer.appendChild(summary);
    if (sourceUsage.complete === false) {
      var unread = document.createElement('div');
      unread.className = 'source-usage-unread';
      var failedIds = unreadIds.concat(missingIds.filter(function(sourceId) { return unreadIds.indexOf(sourceId) < 0; }));
      unread.textContent = '未完整读取：' + failedIds.map(sourceUsageDisplayName).join('、');
      var diagnostics = sourceUsage.diagnostics || [];
      if (diagnostics.length) unread.title = diagnostics.map(function(item) { return item.code || String(item); }).join('；');
      footer.appendChild(unread);
    }
    return footer;
  }

  function addCompletedAssistantReply(text, includeMindmapActions, sourceUsage, answerDerivedWritesEligible) {
    var replyText = String(text || '');
    var sourceUsageActive = !!(sourceUsage && (sourceUsage.active || sourceUsage.sourceCount));
    var allowAnswerDerivedWrites = !!includeMindmapActions && (!sourceUsageActive || (sourceUsage.complete === true && answerDerivedWritesEligible === true));
    ...
    return addMessageWithExtra('assistant', replyText, function(article) {
      var sourceFooter = buildSourceUsageFooter(sourceUsage);
      if (sourceFooter) article.appendChild(sourceFooter);
      var actions = document.createElement('div');
      actions.className = 'reply-completed-actions';
      if (replyText) actions.appendChild(buildReplyCopyButton(replyText));
      if (allowAnswerDerivedWrites) actions.appendChild(buildReplyAgentActions(replyText));
      if (actions.childNodes.length) article.appendChild(actions);
    });
  }

  function addAssistantReplyWithActions(text, sourceUsage, answerDerivedWritesEligible) {
    return addCompletedAssistantReply(text, true, sourceUsage, answerDerivedWritesEligible);
  }
  ...
  function applyMindmapDiffDraftEdits(draftId, panel, done) {
    ...
    if (!state.draft || String(state.draft.id || '') !== String(draftId || '')) return;
    var operationBinding = captureQueuedDraftOperationBinding(state.draft, panel);
    if (!queuedDraftOperationBindingMatchesActiveSession(operationBinding)) return;
    ...
    postCompanion('draft_update', {
        ...
        queueId: operationBinding.queueId,
        conversationId: operationBinding.conversationId,
        sessionId: operationBinding.sessionId,
        sessionEpoch: operationBinding.sessionEpoch,
        contextDocumentKey: operationBinding.contextDocumentKey
      }, function(result) {
        if (!queuedDraftOperationBindingMatchesActiveSession(operationBinding)) return;
        if (!result || !result.ok) {
  ...
  function rejectMindmapDiff(panel) {
    ...
    postCompanion('draft_delete', {
      ...
      if (operationBinding.queueId) resolveQueuedWriteConfirmation(operationBinding);
    }, {showReply: false});
  }
  ...
  function sourceWorkspaceSelectionIds() {
    var ordered = [];
    var seen = {};
    for (var i = 0; i < state.sourceWorkspaceCandidates.length; i++) {
      var candidateId = String((state.sourceWorkspaceCandidates[i] || {}).id || '');
      if (candidateId && state.sourceWorkspaceSelection[candidateId]) {
        ordered.push(candidateId);
        seen[candidateId] = true;
      }
    }
    for (var sourceId in state.sourceWorkspaceSelection) {
      if (Object.prototype.hasOwnProperty.call(state.sourceWorkspaceSelection, sourceId) && state.sourceWorkspaceSelection[sourceId] && !seen[sourceId]) {
  ...
  function sourceWorkspaceSelectionMap(sourceIds) {
    var selection = {};
    sourceIds = sourceIds || [];
    for (var i = 0; i < sourceIds.length; i++) {
      var sourceId = String(sourceIds[i] || '');
      if (sourceId) selection[sourceId] = true;
    }
    return selection;
  }
  ...
  function sourceWorkspaceCandidateById(sourceId) {
    ...
    for (var i = 0; i < state.sourceWorkspaceCandidates.length; i++) {
      var candidate = state.sourceWorkspaceCandidates[i] || {};
      if (String(candidate.id || '') === sourceId) return candidate;
    }
    return null;
  }

  function sourceWorkspaceRecordById(sourceId) {
    var sources = state.sourceWorkspace && state.sourceWorkspace.sources ? state.sourceWorkspace.sources : [];
    for (var i = 0; i < sources.length; i++) {
      var source = sources[i] || {};
      if (String(source.sourceId || source.id || '') === String(sourceId || '')) return source;
    }
    return null;
  }

  function isCurrentDocumentCandidate(candidate) {
    candidate = candidate || {};
    var metadata = candidate.metadata || {};
    var ctx = state.context || {};
    var bookmd5 = String(ctx.bookmd5 || ctx.docmd5 || '');
    var candidateBookmd5 = String(candidate.bookmd5 || metadata.bookmd5 || '');
    if (bookmd5 && candidateBookmd5 && bookmd5 === candidateBookmd5) return true;
    return !candidateBookmd5 && (candidate.kind === 'pdf_cache' || candidate.kind === 'explicit_pdf') &&
      String(candidate.title || '') === String(ctx.documentTitle || ctx.documentFileName || '');
  }

  function currentDocumentSourceCandidate() {
    var fallback = null;
    var cached = null;
    var selected = null;
    for (var i = 0; i < state.sourceWorkspaceCandidates.length; i++) {
      var candidate = state.sourceWorkspaceCandidates[i] || {};
      if (!isCurrentDocumentCandidate(candidate)) continue;
      if (!fallback) fallback = candidate;
      if (!selected && state.sourceWorkspaceSelection[String(candidate.id || '')]) selected = candidate;
  ...
  function sourceWorkspaceCandidateGroup(candidate) {
    candidate = candidate || {};
    if (String(candidate.id || '') === state.sourceWorkspaceCurrentDocumentId || isCurrentDocumentCandidate(candidate)) return 'current';
    if (candidate.kind === 'marginnote_pdf') return 'notebook';
    if (candidate.kind === 'upload') return 'upload';
    return '';
  }

  function sourceWorkspaceKindLabel(candidate) {
    var labels = {
      pdf_cache: 'PDF 缓存',
      marginnote_pdf: 'MarginNote PDF',
      upload: '上传文件'
    };
    var kind = String((candidate || {}).kind || 'file');
    return labels[kind] || kind.replace(/_/g, ' ');
  }

  function sourceWorkspaceSizeLabel(candidate) {
    candidate = candidate || {};
    var size = Number(candidate.size || candidate.byteSize || candidate.fileSize || 0);
    if (!size) return '大小未知';
  ...
  function setSourceWorkspaceStatus(tone, text) {
    state.sourceWorkspaceStatus = {tone: tone || 'neutral', text: text || '资料尚未验证'};
    renderSourceWorkspaceHeaderControl();
    var status = byId('sourceWorkspaceValidationStatus');
    if (status) {
      status.className = 'source-workspace-validation ' + state.sourceWorkspaceStatus.tone;
      status.textContent = state.sourceWorkspaceStatus.text;
    }
  }

  function renderSourceWorkspaceHeaderControl() {
    var selectedCount = sourceWorkspaceSelectionIds().length;
    setText('sourceWorkspaceCount', String(selectedCount));
    var button = byId('sourceWorkspaceButton');
    if (!button) return;
    var tone = state.sourceWorkspaceStatus && state.sourceWorkspaceStatus.tone ? state.sourceWorkspaceStatus.tone : 'neutral';
    if (!selectedCount) tone = 'neutral';
    button.className = 'small-button source-workspace-button ' + tone;
    button.title = selectedCount ? ('当前资料：' + selectedCount + ' 个文件') : '选择资料文件';
  }

  function mergeUnavailableSelectedCandidates(candidates, previousCandidates, selectedIds) {
    candidates = candidates || [];
    previousCandidates = previousCandidates || [];
    selectedIds = selectedIds || [];
    var seen = {};
    for (var i = 0; i < candidates.length; i++) seen[String((candidates[i] || {}).id || '')] = true;
    for (var s = 0; s < selectedIds.length; s++) {
      var selectedId = String(selectedIds[s] || '');
      if (!selectedId || seen[selectedId]) continue;
      var previous = null;
      for (var p = 0; p < previousCandidates.length; p++) {
        if (String((previousCandidates[p] || {}).id || '') === selectedId) previous = previousCandidates[p];
      }
      candidates.push(Object.assign({}, previous || {id: selectedId, title: selectedId, kind: 'unavailable'}, {
        readable: false,
        unavailable: true
      }));
  ...
  function applySourceWorkspaceResult(result, options) {
    result = result || {};
    options = options || {};
    var previousCandidates = state.sourceWorkspaceCandidates.slice();
    var sourceIds = options.selectionOverride || result.sourceIds || sourceWorkspaceSelectionIds();
    var candidatesPayload = result.candidates || {};
    var candidates = candidatesPayload.sources || result.sourceWorkspaceCandidates || [];
    state.sourceWorkspaceCandidates = mergeUnavailableSelectedCandidates(candidates.slice(), previousCandidates, sourceIds);
    state.sourceWorkspaceSelection = sourceWorkspaceSelectionMap(sourceIds);
    ...
    var errors = result.errors || state.sourceWorkspace.errors || [];
    var selectedCount = sourceWorkspaceSelectionIds().length;
    var extractionWarnings = (state.sourceWorkspace.sources || []).filter(function(source) {
      return !!((source || {}).error || (source || {}).truncated);
    });
    if (errors.length || result.ok === false) {
      setSourceWorkspaceStatus('error', '资料不可用：' + (errors[0] || result.message || '验证失败'));
    } else if (state.sourceWorkspaceInFlight) {
      setSourceWorkspaceStatus('warning', '正在验证资料...');
    } else if (selectedCount && state.sourceWorkspace.revision && state.sourceWorkspace.active !== false && extractionWarnings.length) {
      setSourceWorkspaceStatus('warning', '已验证 ' + selectedCount + ' 个文件；' + extractionWarnings.length + ' 个有文本提取诊断');
    } else if (selectedCount && state.sourceWorkspace.revision && state.sourceWorkspace.active !== false) {
      setSourceWorkspaceStatus('ready', '已验证 ' + selectedCount + ' 个文件，均可读');
    } else if (selectedCount) {
      setSourceWorkspaceStatus('warning', '已选择 ' + selectedCount + ' 个文件，等待验证');
    } else {
      setSourceWorkspaceStatus('neutral', '未选择资料文件');
    }
    renderSourceWorkspacePage();
  }

  function sourceWorkspaceEmptyRow(text) {
    var empty = document.createElement('div');
    empty.className = 'source-workspace-empty';
    empty.textContent = text;
    return empty;
  }

  function buildSourceWorkspaceRow(candidate) {
    candidate = candidate || {};
    ...
    body.className = 'source-workspace-row-body';
    var name = document.createElement('strong');
    name.className = 'source-workspace-name';
    name.textContent = String(candidate.title || sourceId || '未命名文件');
    var meta = document.createElement('span');
    meta.className = 'source-workspace-meta';
    var readable = candidate.unavailable || candidate.readable === false || workspaceSource.readable === false ? '不可读' : '可读';
    var extraction = workspaceSource.textReadable === true || workspaceSource.textLink ?
      (workspaceSource.truncated ? '文本已提取（已截断）' : '文本已提取') :
      (workspaceSource.error ? '文本提取失败' : '文本状态待验证');
    meta.textContent = sourceWorkspaceKindLabel(candidate) + ' / ' + sourceWorkspaceSizeLabel(candidate) + ' / ' + readable + ' / ' + extraction;
    if (workspaceSource.error) meta.title = String(workspaceSource.error);
  ...
  function renderSourceWorkspaceDiagnostics() {
    var details = byId('sourceWorkspaceDiagnostics');
    var body = byId('sourceWorkspaceDiagnosticsBody');
    if (!details || !body || !details.open) return;
    var lines = [];
    for (var i = 0; i < state.sourceWorkspaceCandidates.length; i++) {
      var candidate = state.sourceWorkspaceCandidates[i] || {};
      if (!state.sourceWorkspaceSelection[String(candidate.id || '')]) continue;
      lines.push((candidate.title || candidate.id || 'source') + ': ' + (candidate.path || '路径不可用'));
    }
    var diagnostics = state.sourceWorkspace && state.sourceWorkspace.errors ? state.sourceWorkspace.errors : [];
    if (diagnostics.length) lines.push('验证：' + diagnostics.join('；'));
    var workspaceSources = state.sourceWorkspace && state.sourceWorkspace.sources ? state.sourceWorkspace.sources : [];
    for (var sourceIndex = 0; sourceIndex < workspaceSources.length; sourceIndex++) {
      var workspaceSource = workspaceSources[sourceIndex] || {};
      if (workspaceSource.error) {
        lines.push((workspaceSource.displayName || workspaceSource.sourceId || 'source') + ' 文本提取：' + workspaceSource.error);
      }
      if (workspaceSource.truncated) {
        lines.push((workspaceSource.displayName || workspaceSource.sourceId || 'source') + '：提取文本已截断');
      }
    }
  ...
  function renderSourceWorkspacePage() {
    var groups = {current: [], notebook: [], upload: []};
    for (var i = 0; i < state.sourceWorkspaceCandidates.length; i++) {
      var candidate = state.sourceWorkspaceCandidates[i] || {};
      if (isCurrentDocumentCandidate(candidate) && String(candidate.id || '') !== state.sourceWorkspaceCurrentDocumentId && !state.sourceWorkspaceSelection[String(candidate.id || '')]) continue;
      var group = sourceWorkspaceCandidateGroup(candidate);
      if (group && groups[group]) groups[group].push(buildSourceWorkspaceRow(candidate));
    }
    var targets = {
      current: byId('sourceWorkspaceCurrentDocumentList'),
      notebook: byId('sourceWorkspaceNotebookList'),
      upload: byId('sourceWorkspaceUploadList')
    };
    var emptyText = {
      current: '当前文件尚不可用。',
      notebook: '当前 notebook 没有其他可读文件。',
      upload: '还没有已上传文件。'
    };
    for (var key in targets) {
      if (!Object.prototype.hasOwnProperty.call(targets, key) || !targets[key]) continue;
      replaceElementChildren(targets[key], groups[key].length ? groups[key] : [sourceWorkspaceEmptyRow(emptyText[key])]);
    }
    ...
    if (statusNode) {
      statusNode.className = 'source-workspace-validation ' + status.tone;
  ...
  function sourceWorkspaceOperationAllowed(lifecycleHandle) {
    return sourceWorkspaceLifecycle.isSourceMutationAllowed(lifecycleHandle);
  }
  ...
  function saveSourceWorkspaceSelection(closeAfterSave, done, lifecycleHandle) {
    ...
    ensureSourceWorkspaceConversation(function() {
      ...
      postSourceWorkspace('source_workspace_update', {
        ...
        if (result && result.ok) {
          if (closeAfterSave) closeSourceWorkspacePage();
        } else {
    ...
    ensureSourceWorkspaceConversation(function() {
      ...
      postSourceWorkspace('source_workspace_get', {conversationId: state.conversationId}, function(result) {
        ...
        if (result && result.ok && selectCurrentByDefault && current && !sourceWorkspaceSelectionIds().length && state.followCurrentDocument) {
          ...
          saveSourceWorkspaceSelection(false, null, options.lifecycleHandle);
          return;
        }
  ...
  function clearSourceWorkspace(closeAfterClear, done, lifecycleHandle) {
    if (sourceWorkspaceLifecycle.isUploadActive() && !sourceWorkspaceLifecycle.isUploadCurrent(lifecycleHandle)) {
      cancelSourceWorkspaceUpload('clear');
    }
    if (!sourceWorkspaceOperationAllowed(lifecycleHandle)) return;
  ...
  function uploadSourceWorkspaceFiles(fileList) {
    ...
    if (sourceWorkspaceLifecycle.isUploadActive()) {
      renderSourceWorkspaceUploadStatus('已有文件正在上传，请等待当前批次结束。', []);
      return;
    }
  ...
  function openSourceWorkspacePage() {
    if (!sourceWorkspaceOperationAllowed(null)) {
      addMessage(
        'assistant',
        sourceWorkspaceLifecycle.isUploadActive() ?
          '资料文件正在上传，当前批次结束前不能重新打开或修改资料。' :
    ...
    if (page) page.className = 'config-page source-workspace-page';
  ...
  function clearQueuedWriteConfirmationUi() {
    state.pendingQueuedWriteConfirmation = null;
    state.pendingAiEditDrafts = {};
    state.draft = null;
    state.aiEditTransactionStatus = {schema: 'codex.mn.aiEditTransactionStatus.v1', available: false};
    renderDraft(null);
    renderAiEditTransactionCenter(state.aiEditTransactionStatus);
    var panels = document.querySelectorAll(
      '.ai-edit-operation, .mindmap-diff-operation, .operation-plan-panel'
    );
    for (var i = 0; i < panels.length; i++) {
      if (panels[i] && panels[i].parentNode) panels[i].parentNode.removeChild(panels[i]);
    }
  }

  function resetQueueRuntimeForConversationSwitch() {
    state.queueRuntimeReady = false;
    if (state.queueSessionRestoreRetryTimer) window.clearTimeout(state.queueSessionRestoreRetryTimer);
    state.queueSessionRestoreRetryTimer = null;
    state.queueSessionRestoreToken += 1;
    state.queueSessionRestoreInFlight = false;
    state.queueSessionRestoreContextKey = '';
    state.queueSessionRestoredContextKey = '';
    state.queueSessionRestoreComplete = false;
    state.queueGuardRestoreToken += 1;
    state.queueGuardRestoreInFlight = false;
    state.queueGuardRestoreIdentityKey = '';
  ...
  function setCurrentConversation(conversation, options) {
    ...
    state.queueSessionRestoredContextKey = String(state.contextDocumentKey || '');
    if (conversation.sourceIds !== undefined) {
      state.sourceWorkspaceSelection = sourceWorkspaceSelectionMap(conversation.sourceIds || []);
    }
    if (conversation.followCurrentDocument !== undefined) {
      state.followCurrentDocument = !!conversation.followCurrentDocument;
    }
    if (conversation.sourceWorkspaceRevision !== undefined) {
      state.sourceWorkspace.revision = String(conversation.sourceWorkspaceRevision || '');
    }
    renderSourceWorkspaceHeaderControl();
    if (!options.deferQueuePreparation) prepareQueueRuntimeForActiveSession();
  }
  ...
  function companionPayload(action, extra) {
    ...
    var ctx = state.context || {};
    for (var key in ctx) {
      if (Object.prototype.hasOwnProperty.call(ctx, key) && key !== 'webActionToken') payload[key] = ctx[key];
    }
    extra = extra || {};
    var hasExplicitConversation = Object.prototype.hasOwnProperty.call(extra, 'conversationId');
    var hasExplicitSession = Object.prototype.hasOwnProperty.call(extra, 'sessionId');
  ...
  function displayCompanionResult(result, showReply, action) {
    ...
    if (showReply && result.reply) {
      if (result.ok === true && (action === 'chat' || action === 'explain_selection' || action === 'goal_run')) {
        addAssistantReplyWithActions(result.reply, result.sourceUsage, result.answerDerivedWritesEligible);
      } else {
  ...
  function postCompanion(action, extra, done, options) {
    ...
    if (Number(options.timeoutMs || 0) > 0) xhr.timeout = Number(options.timeoutMs);
    ...
    function routeStaleResponse(result) {
      var staleDocument = !!(
        requestDocumentKey &&
        state.contextDocumentKey &&
        requestDocumentKey !== state.contextDocumentKey
      );
      var staleBinding = options.requireActiveBinding === true &&
        !window.SourceWorkspaceLifecycle.requestBindingMatchesActiveSession(requestPayload, state);
      if (!staleDocument && !staleBinding) return false;
      if (options.onStaleResponse) options.onStaleResponse(result || {}, requestPayload);
      return true;
    }
    xhr.onreadystatechange = function() {
      if (xhr.readyState !== 4 || requestSettled) return;
      requestSettled = true;
      var parsedResult = parseCompanionResult(xhr);
      if (routeStaleResponse(parsedResult)) return;
    ...
    xhr.onerror = function() {
      ...
      if (routeStaleResponse(result)) return;
    ...
    xhr.ontimeout = function() {
      if (requestSettled) return;
      requestSettled = true;
      var result = {ok: false, httpStatus: 0, message: 'Companion 请求超时，请稍后重试。'};
      if (routeStaleResponse(result)) return;
      if (done) done(result);
    };
  ...
  function ackQueueAndContinue(queueId) {
    ...
    if (queueId) {
      ackQueuedCommands([queueId], function(result) {
        if (!result || !result.ok) {
          state.completedAckPendingQueueIds[queueId] = true;
          window.CodexPanel.setStatus({text: '队列结果已完成，但确认回执失败；将只重试回执，不会再次执行。'});
          return;
        }
        delete state.completedAckPendingQueueIds[queueId];
        refreshQueue();
  ...
  function restoreQueueRuntimeSessionForContext(done) {
    var contextDocumentKey = String(state.contextDocumentKey || '');
    if (!documentContextReadyForAutomaticSwitch(state.context || {}, contextDocumentKey)) {
      if (done) done(false);
      return;
    }
    if (
      state.queueSessionRestoreComplete &&
      state.queueSessionRestoredContextKey === contextDocumentKey
    ) {
      if (done) done(true);
      return;
    }
    if (
      state.queueSessionRestoreInFlight &&
      state.queueSessionRestoreContextKey === contextDocumentKey
    ) return;
    ...
    state.queueSessionRestoreRetryTimer = null;
    state.queueSessionRestoreToken += 1;
    var restoreToken = state.queueSessionRestoreToken;
    state.queueSessionRestoreInFlight = true;
    state.queueSessionRestoreContextKey = contextDocumentKey;
    postCompanion('conversation_active_restore', {
      contextDocumentKey: contextDocumentKey,
      mnObject: {},
      mnObjectId: ''
    }, function(result) {
      if (
        restoreToken !== state.queueSessionRestoreToken ||
        contextDocumentKey !== String(state.contextDocumentKey || '')
      ) return;
      state.queueSessionRestoreInFlight = false;
      state.queueSessionRestoreContextKey = '';
      if (!result || !result.ok || result.restoreComplete !== true) {
        state.queueSessionRestoreComplete = false;
        state.queueSessionRestoredContextKey = '';
        state.queueSessionRestoreRetryTimer = window.setTimeout(function() {
          if (
            restoreToken !== state.queueSessionRestoreToken ||
            contextDocumentKey !== String(state.contextDocumentKey || '')
          ) return;
      ...
      var conversation = result.active === true ? (result.conversation || {}) : {};
      setCurrentConversation(conversation, {deferQueuePreparation: true});
      renderHistoryItems(result.history || []);
      ...
      if (done) done(true);
    }, {showReply: false, timeoutMs: 8000});
  }
  ...
  function queuedSessionIdentity() {
    return {
      ...
      sessionEpoch: String(state.sessionEpoch || ''),
      contextDocumentKey: String(state.contextDocumentKey || ''),
      queueSessionRestoreComplete: state.queueSessionRestoreComplete === true
    };
  }
  ...
  function restoreQueuedWriteConfirmationGuard(done) {
    ...
    state.queueGuardRestoreIdentityKey = identityKey;
    postCompanion('queued_write_confirmation_get', identity, function(result) {
      ...
      var confirmation = result.confirmation || null;
      if (
        confirmation &&
  ...
  function prepareQueueRuntimeForActiveSession(done) {
    state.queueRuntimeReady = false;
    stopQueuePump();
    var identity = queuedSessionIdentity();
    var identityKey = queuedSessionIdentityKey(identity);
    if (!window.SourceWorkspaceLifecycle.queueRuntimeCanStart(identity)) {
      if (done) done(false);
      return;
    }
    if (!identityKey) {
      ...
      state.queueGuardRestoreIdentityKey = '';
      state.queueGuardRestoredIdentityKey = '';
      state.queueRuntimeReady = true;
      startQueuePump();
      if (done) done(true);
      return;
    }
    restoreQueuedWriteConfirmationGuard(function(restored) {
      if (
        !restored ||
        identityKey !== queuedSessionIdentityKey()
      ) {
        if (done) done(false);
        return;
      }
  ...
  function persistQueuedWriteConfirmationState(identity, status, done) {
    ...
    var payload = {
      ...
      sessionEpoch: String(identity.sessionEpoch || ''),
      contextDocumentKey: String(identity.contextDocumentKey || '')
    };
  ...
  function persistCompletedQueuedResult(command, done) {
    ...
    if (!queueId) {
      if (done) done({ok: true, updated: 0});
      return;
    }
    postCompanionPath('/marginnote/queue-complete', 'queue_complete', {ids: [queueId]}, function(result) {
      if (done) done(result || {ok: false, message: '队列完成状态持久化失败。'});
    });
  }
  ...
  function applyQueuedResultPolicy(command, result, effects) {
    effects = effects || {};
      ...
      onDeferred: function(detail) {
        detail.silent = effects.silent === true;
        deferQueuedResult(command, detail.result || result, detail);
        if (effects.onDeferred) effects.onDeferred(detail);
      },
      onAck: function() {
        var queueId = String((command || {})._queue_id || '');
        persistCompletedQueuedResult(command, function(persisted) {
          if (!persisted || !persisted.ok) {
            deferQueuedResult(command, persisted || {}, {
              status: 'deferred',
              reason: 'completion_persist_failed',
              retryable: true,
              routing: window.SourceWorkspaceLifecycle.queuedSessionRouting(command, state),
              result: persisted || {},
              silent: effects.silent === true
            });
            return;
          }
          if (queueId) {
            delete state.deferredQueueResults[queueId];
            state.completedAckPendingQueueIds[queueId] = true;
          }
  ...
  function saveQueuedWriteForConfirmation(command, result, done) {
    ...
    postCompanionPath('/marginnote/draft', 'draft_save', {
      ...
      writeTarget: result.writeTarget || (result.mindmap && result.mindmap.writeTarget) || command.writeTarget || {}
    }, function(saved) {
      ...
      if (
        ...
        String(command.contextDocumentKey || '') !== String(state.contextDocumentKey || '')
      ) {
  ...
  function freezeQueuedDispatchValue(value) {
    if (!value || typeof value !== 'object' || !Object.freeze || Object.isFrozen(value)) return value;
    for (var key in value) {
      if (Object.prototype.hasOwnProperty.call(value, key)) freezeQueuedDispatchValue(value[key]);
    }
    return Object.freeze(value);
  }

  function captureQueuedCommandDispatch(command) {
    command = command || {};
    var captured = {};
    try {
      captured = JSON.parse(JSON.stringify(command));
    } catch (err) {
      for (var key in command) {
        if (Object.prototype.hasOwnProperty.call(command, key)) captured[key] = command[key];
      }
    }
    captured._queue_id = String(command._queue_id || captured._queue_id || '');
    captured.queueId = captured._queue_id;
    captured.conversationId = String(command.conversationId || '');
    captured.sessionId = String(command.sessionId || '');
    captured.sessionEpoch = String(command.sessionEpoch || '');
    captured.contextDocumentKey = String(command.contextDocumentKey || '');
    return freezeQueuedDispatchValue(captured);
  }

  function runQueuedCommand(command, options) {
    command = captureQueuedCommandDispatch(command || {});
    options = options || {};
    ...
    if (rawAction === 'goal_run') {
      requestGoalAction(prompt, '[队列执行] 目标', queueId, extraPayload, command);
      return;
    }
    if (isWriteAction(rawAction)) {
      requestDraftAction(rawAction, prompt, '[队列执行] ' + actionLabel(rawAction), queueId, extraPayload, command);
      return;
    }
    requestTextAction(rawAction, prompt, '[队列执行] ' + actionLabel(rawAction), queueId, extraPayload, command);
  }

  function drainNextQueuedAction(options) {
    options = options || {};
    if (!state.queueRuntimeReady) return;
    ...
    xhr.onreadystatechange = function() {
      ...
      var commands = result.commands && result.commands.length ? result.commands.slice() : [];
      if (result.command) {
        var primaryQueueId = String(result.command._queue_id || '');
        var primaryIncluded = false;
        for (var commandIndex = 0; commandIndex < commands.length; commandIndex++) {
          if (String((commands[commandIndex] || {})._queue_id || '') === primaryQueueId) {
            primaryIncluded = true;
            break;
          }
        }
        if (!primaryIncluded) commands.unshift(result.command);
      }
      var command = window.SourceWorkspaceLifecycle.firstRunnableQueuedCommand(
        commands,
        state,
        Object.assign({}, options, {isWriteAction: isWriteAction})
      );
      if (!command || result.blocked) return;
      var queueId = String(command._queue_id || '');
      var executionDisposition = window.SourceWorkspaceLifecycle.queuedExecutionDisposition(command, state);
      if (executionDisposition === 'ack_only') {
        state.completedAckPendingQueueIds[queueId] = true;
        ackQueueAndContinue(queueId);
        return;
      }
      if (executionDisposition === 'confirmation_pending') return;
  ...
  function requestTextAction(action, prompt, userText, queueId, extraPayload, queuedDispatch) {
    ...
    var queuedCommand = queueId ? (queuedDispatch || captureQueuedCommandDispatch(Object.assign({}, extraPayload, {
      _queue_id: queueId,
      rawAction: action
    }))) : null;
  ...
  function sourceWorkspaceGenerationUnavailableReason() {
    ...
    var workspace = state.sourceWorkspace || {};
    if (!workspace.revision || workspace.active === false) return '资料选择尚未保存并验证，请打开“资料”后点“完成”。';
    var workspaceIds = [];
    var sources = workspace.sources || [];
    for (var i = 0; i < sources.length; i++) workspaceIds.push(String((sources[i] || {}).sourceId || (sources[i] || {}).id || ''));
    if (workspaceIds.join('|') !== sourceIds.join('|')) return '资料选择已更改，请先保存并验证后再发送。';
    if (workspace.errors && workspace.errors.length) return '资料验证失败，请移除不可读文件后重试。';
    return '';
  }

  function addSourceRequestStatus() {
    var count = sourceWorkspaceSelectionIds().length;
    if (!count) return;
    addMessageWithExtra('assistant', '本次资料：' + count + ' 个文件，均可读', function(article) {
      article.className = 'message assistant source-request-status';
    });
  }
  ...
  function sourceWorkspaceGenerationAction(action) {
    return action === 'chat' || action === 'explain_selection' || action === 'goal_run' || isWriteAction(action);
  }

  function actionUnavailableReason(action, prompt) {
    var sourceWorkspaceReason = sourceWorkspaceGenerationUnavailableReason();
    if (sourceWorkspaceReason && sourceWorkspaceGenerationAction(action)) return sourceWorkspaceReason;
  ...
  function buildAiEditOperationPanel(draft) {
    ...
    panel.setAttribute('data-queue-id', draft && draft.queueId ? draft.queueId : '');
    var title = document.createElement('div');
  ...
  function requestGoalAction(goalText, userText, queueId, extraPayload, queuedDispatch) {
    state.currentQueueId = queueId || '';
    extraPayload = extraPayload || {};
    if (generationLifecycleUnavailableReason()) {
      if (queueId) deferQueuedGenerationForLifecycle();
    ...
    if (!goal.title && !goal.detail) {
      if (queueId) {
        applyQueuedResultPolicy(queuedCommand, {
          ok: false,
          blocked: 'empty_queued_goal',
          message: '队列目标为空，已保留并等待修正或重试。'
        });
      } else {
        addMessage('assistant', '目标为空，无法执行。');
      }
      return;
    }
  ...
  function requestDraftAction(action, prompt, userText, queueId, extraPayload, queuedDispatch) {
    state.currentQueueId = queueId || '';
    extraPayload = extraPayload || {};
    if (generationLifecycleUnavailableReason()) {
      if (queueId) deferQueuedGenerationForLifecycle();
    ...
    if (!ensureMindmapTargetReady(action)) {
      if (queueId) {
        applyQueuedResultPolicy(queuedDispatch || captureQueuedCommandDispatch(Object.assign({}, extraPayload, {
          _queue_id: queueId,
          rawAction: action
        })), {
          ok: false,
    ...
    postCompanion(action, requestPayload, function(result) {
      ...
      if (result.sourceUsage && (result.sourceUsage.complete !== true || result.answerDerivedWritesEligible !== true)) {
        ...
        reportActionResponse(action, result || {});
        addCompletedAssistantReply(result.reply || '', false, result.sourceUsage, false);
        finishProgressStage('资料读取不完整', '回答已保留，但已阻止卡片或脑图写入。');
        ackQueueAndContinue(queueId);
        return;
      }
      if (!result.cards && !result.mindmap) {
  ...
  function executeAction(action, prompt, userText, extraPayload) {
    ...
    if (action === 'chat' || action === 'explain_selection') {
      requestTextAction(action, prompt, userText || '[' + action + ']', '', extraPayload);
      return true;
    }
    requestTextAction(action, prompt, '[' + action + '] ' + (userText || '当前内容'), '', extraPayload);
    return true;
  }
  ...
  function startQueuePump() {
    ...
    state.queuePumpStartTimer = window.setTimeout(function() {
      state.queuePumpStartTimer = null;
      queuePumpTick();
    }, 600);
  }

  function stopQueuePump() {
    if (state.queuePumpTimer) {
      window.clearInterval(state.queuePumpTimer);
      state.queuePumpTimer = null;
    }
    if (state.queuePumpStartTimer) {
      window.clearTimeout(state.queuePumpStartTimer);
      state.queuePumpStartTimer = null;
    }
  }
  ...
  function resolveQueuedWriteConfirmation(identity) {
    identity = identity || {};
    ...
    if (
      (identity.conversationId && String(identity.conversationId) !== String(pending.conversationId || '')) ||
      (identity.sessionId && String(identity.sessionId) !== String(pending.sessionId || '')) ||
      (identity.sessionEpoch && String(identity.sessionEpoch) !== String(pending.sessionEpoch || '')) ||
      (identity.contextDocumentKey && String(identity.contextDocumentKey) !== String(pending.contextDocumentKey || ''))
    ) return false;
  ...
  function captureQueuedDraftOperationBinding(draft, panel) {
    draft = draft || {};
    var draftId = String(draft.id || draft.draftId || '');
    var pending = state.pendingQueuedWriteConfirmation || {};
    var queueId = String(draft.queueId || '');
    if (!queueId && draftId && String(pending.draftId || '') === draftId) {
      queueId = String(pending.queueId || '');
    }
    var confirmation = draft.queueConfirmation || {};
    if (
      queueId &&
      String(pending.queueId || '') === queueId &&
      String(pending.draftId || '') === draftId
    ) confirmation = pending;
    var transactionId = currentAiEditTransactionId(panel);
    var binding = {
      queueId: queueId,
      draftId: draftId,
      transactionId: String(transactionId || confirmation.transactionId || ''),
      conversationId: String(queueId ? confirmation.conversationId || '' : state.conversationId || ''),
      sessionId: String(queueId ? confirmation.sessionId || '' : state.sessionId || ''),
      sessionEpoch: String(queueId ? confirmation.sessionEpoch || '' : state.sessionEpoch || ''),
      contextDocumentKey: String(queueId ? confirmation.contextDocumentKey || '' : state.contextDocumentKey || ''),
      draft: Object.assign({}, draft, {
        queueId: queueId,
        queueConfirmation: Object.assign({}, confirmation)
      })
    };
    return freezeQueuedDispatchValue(binding);
  }

  function queuedDraftOperationResultMatchesBinding(candidate, binding) {
    candidate = candidate || {};
    binding = binding || {};
    var fields = [
      'queueId',
    ...
    for (var i = 0; i < fields.length; i++) {
      var expected = String(binding[fields[i]] || '');
      if (expected && expected !== String(candidate[fields[i]] || '')) return false;
    }
    return true;
  }

  function queuedDraftOperationBindingMatchesActiveSession(binding) {
    binding = binding || {};
    var draftId = String(binding.draftId || '');
    if (!draftId) return false;
    if (!String(binding.queueId || '')) {
      return !!(
        state.draft &&
        String(state.draft.id || '') === draftId &&
        String(binding.conversationId || '') === String(state.conversationId || '') &&
        String(binding.sessionId || '') === String(state.sessionId || '') &&
        String(binding.sessionEpoch || '') === String(state.sessionEpoch || '') &&
        String(binding.contextDocumentKey || '') === String(state.contextDocumentKey || '')
      );
    }
    if (!window.SourceWorkspaceLifecycle.queuedConfirmationMatchesActiveSession(binding, state)) return false;
    var pending = state.pendingQueuedWriteConfirmation;
    if (!pending) return false;
    if (!queuedDraftOperationResultMatchesBinding(pending, binding)) return false;
    if (
      binding.transactionId && pending.transactionId &&
      String(binding.transactionId) !== String(pending.transactionId)
    ) return false;
    return true;
  }

  function writeAcceptedDraft(operationBinding, panel) {
    if (typeof operationBinding === 'string') {
      if (!state.draft || String(state.draft.id || '') !== String(operationBinding || '')) return;
      operationBinding = captureQueuedDraftOperationBinding(state.draft, panel);
    }
    operationBinding = operationBinding || {};
    if (!queuedDraftOperationBindingMatchesActiveSession(operationBinding)) return;
    var draftId = String(operationBinding.draftId || '');
    var queueId = String(operationBinding.queueId || '');
    function sendWriteRequest() {
      if (!queuedDraftOperationBindingMatchesActiveSession(operationBinding)) return;
      if (queueId) {
        state.pendingAiEditDrafts[draftId] = Object.assign({}, operationBinding.draft || {}, {queueId: queueId});
      }
      bridge('write_draft', {
        id: draftId,
        draftId: draftId,
        queueId: queueId,
        conversationId: String(operationBinding.conversationId || ''),
        sessionId: String(operationBinding.sessionId || ''),
        sessionEpoch: String(operationBinding.sessionEpoch || ''),
        contextDocumentKey: String(operationBinding.contextDocumentKey || '')
      });
      renderDraft(null);
      if (panel) {
    ...
    persistQueuedWriteConfirmationState(operationBinding, 'native_write', function(result) {
      if (!queuedDraftOperationBindingMatchesActiveSession(operationBinding)) return;
      if (!result || !result.ok) {
        setAiEditOperationBusy(panel, false);
      ...
      if (!queuedDraftOperationResultMatchesBinding(result.confirmation || {}, operationBinding)) {
        setAiEditOperationBusy(panel, false);
        setAiEditOperationStatus(panel, '写入确认状态归属不匹配。', 'error');
        return;
      }
  ...
  function queuedDraftBindingMatchesActiveSession(draft) {
    draft = draft || {};
    if (!draft.queueId) return true;
    var confirmation = draft.queueConfirmation || {};
    if (
      !confirmation.sessionId &&
      state.pendingQueuedWriteConfirmation &&
      String(state.pendingQueuedWriteConfirmation.queueId || '') === String(draft.queueId || '')
    ) confirmation = state.pendingQueuedWriteConfirmation;
    return window.SourceWorkspaceLifecycle.queuedDraftBindingMatchesActiveSession(
      Object.assign({}, draft, {queueConfirmation: confirmation}),
      state
    );
  }
  ...
  function acceptDraft(panel) {
    ...
    if (transactionId) {
      ...
      bridge('accept_ai_edit_transaction', {
        transactionId: transactionId,
        queueId: operationBinding.queueId,
        draftId: operationBinding.draftId,
        conversationId: operationBinding.conversationId,
        sessionId: operationBinding.sessionId,
        sessionEpoch: operationBinding.sessionEpoch,
        contextDocumentKey: operationBinding.contextDocumentKey
    ...
    if (!editor || !String(editText || '').replace(/^\s+|\s+$/g, '')) {
      writeAcceptedDraft(operationBinding, panel);
      return;
    }
    setAiEditOperationBusy(panel, true);
  ...
  function rejectDraft(panel) {
    ...
    if (transactionId) {
      ...
      bridge('reject_ai_edit_transaction', {
        transactionId: transactionId,
        queueId: operationBinding.queueId,
        draftId: operationBinding.draftId,
        conversationId: operationBinding.conversationId,
    ...
    var draftId = operationBinding.draftId;
    var queueId = operationBinding.queueId;
    if (draftId) {
      postCompanion('draft_delete', {
        id: draftId,
        queueId: queueId,
        ...
        if (!result || !result.ok) {
          setAiEditOperationBusy(panel, false);
          setAiEditOperationStatus(panel, result && result.message ? result.message : '草稿删除失败。', 'error');
          addFailureMessage('草稿删除失败', result);
          return;
        }
        renderDraft(null);
        if (panel) {
          setAiEditOperationBusy(panel, true);
          setAiEditOperationStatus(panel, '已拒绝并丢弃草稿。', 'rejected');
        } else {
          addMessage('assistant', '已丢弃草稿。');
        }
        resolveQueuedWriteConfirmation(operationBinding);
      });
      return;
    }
    renderDraft(null);
  ...
  function cancelAutomaticDocumentSwitch() {
    ...
    var restoredOriginalState = false;
  ...
  function renderContext(ctx) {
    ...
    for (var contextKey in ctx) {
      if (Object.prototype.hasOwnProperty.call(ctx, contextKey) && contextKey !== 'webActionToken') {
        safeContext[contextKey] = ctx[contextKey];
      }
    }
    state.context = repairContextPayload(safeContext);
    ...
    var queueIdentityKey = queuedSessionIdentityKey();
    if (
      ...
      state.queueSessionRestoreComplete &&
      (
        !state.queueRuntimeReady ||
        (queueIdentityKey && state.queueGuardRestoredIdentityKey !== queueIdentityKey)
      ) &&
      state.queueGuardRestoreIdentityKey !== queueIdentityKey
    ) {
      prepareQueueRuntimeForActiveSession();
    }
    renderContextSourceLine(state.context);
  ...
  window.CodexPanel = {
    ...
    setAiEditOperationReady: function(payload) {
      ...
      var queueId = String(payload.queueId || '');
      if (queueId) {
        var callbackBinding = {
          queueId: queueId,
          draftId: draftId,
          ...
          contextDocumentKey: String(payload.contextDocumentKey || ''),
          status: 'transaction_confirmation'
        };
        if (!window.SourceWorkspaceLifecycle.queuedConfirmationMatchesActiveSession(callbackBinding, state)) return;
        var knownConfirmation = state.pendingQueuedWriteConfirmation || {};
        if (!queuedDraftOperationResultMatchesBinding(knownConfirmation, callbackBinding)) return;
        callbackBinding.conversationId = String(knownConfirmation.conversationId || callbackBinding.conversationId || '');
        ...
        var confirmationBinding = freezeQueuedDispatchValue(Object.assign({}, callbackBinding));
        state.pendingQueuedWriteConfirmation = confirmationBinding;
        draft.queueConfirmation = Object.assign({}, confirmationBinding);
        persistQueuedWriteConfirmationState(
          confirmationBinding,
          'transaction_confirmation',
          function(result) {
            if (
              queuedDraftOperationBindingMatchesActiveSession(confirmationBinding) &&
              result && result.ok && result.confirmation &&
              queuedDraftOperationResultMatchesBinding(result.confirmation, confirmationBinding)
            ) {
              state.pendingQueuedWriteConfirmation = result.confirmation;
            }
          }
      ...
      var directDraft = draftId && state.pendingAiEditDrafts ? state.pendingAiEditDrafts[draftId] : null;
      if (!directDraft) return;
      for (var directKey in payload) {
        if (Object.prototype.hasOwnProperty.call(payload, directKey)) directDraft[directKey] = payload[directKey];
      }
      if (!directDraft.id) directDraft.id = draftId;
      delete state.pendingAiEditDrafts[draftId];
      renderAiEditOperation(directDraft);
    },
    setAiEditOperationResult: function(payload) {
      payload = payload || {};
      var transactionId = String(payload.transactionId || '');
      var queueId = String(payload.queueId || '');
      var resultBinding = null;
      if (queueId) {
        resultBinding = {
          queueId: queueId,
          draftId: String(payload.draftId || payload.id || ''),
          transactionId: transactionId,
          conversationId: String(payload.conversationId || ''),
          sessionId: String(payload.sessionId || ''),
          sessionEpoch: String(payload.sessionEpoch || ''),
        ...
        if (!window.SourceWorkspaceLifecycle.queuedConfirmationMatchesActiveSession(resultBinding, state)) return;
        var activeConfirmation = state.pendingQueuedWriteConfirmation || {};
        if (!queuedDraftOperationResultMatchesBinding(activeConfirmation, resultBinding)) return;
        if (
          resultBinding.transactionId && activeConfirmation.transactionId &&
          resultBinding.transactionId !== String(activeConfirmation.transactionId || '')
        ) return;
        resultBinding.conversationId = String(activeConfirmation.conversationId || resultBinding.conversationId || '');
        resultBinding = freezeQueuedDispatchValue(resultBinding);
      }
      ...
      if (payload.ok && (payload.action === 'accept' || payload.action === 'reject')) {
        var resolvedIdentity = resultBinding || {
          queueId: '',
          draftId: String(payload.draftId || payload.id || ''),
          transactionId: transactionId
        };
        var pendingConfirmation = resultBinding ? state.pendingQueuedWriteConfirmation : null;
        if (pendingConfirmation) {
          persistQueuedWriteConfirmationState(
            resolvedIdentity,
            'resolved',
            function(saved) {
              if (!queuedDraftOperationBindingMatchesActiveSession(resolvedIdentity)) return;
              if (!saved || !saved.ok) {
                window.CodexPanel.setStatus({text: saved && saved.message ? saved.message : '写入确认完成状态保存失败。'});
                return;
              }
              if (saved.confirmation && !queuedDraftOperationResultMatchesBinding(saved.confirmation, resolvedIdentity)) return;
  ...
  function bind() {
    ...
    var sourceWorkspaceDiagnostics = byId('sourceWorkspaceDiagnostics');
    if (sourceWorkspaceDiagnostics) sourceWorkspaceDiagnostics.addEventListener('toggle', renderSourceWorkspaceDiagnostics);
