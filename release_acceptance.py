...
DEFAULT_PACKAGE = ROOT / "release/CodexCompanion-0.4.8-latest-dist.zip"
...
def next_actions_for_gate(name: str, detail: str = "") -> list[str]:
    ...
    if name == "syntax_checks":
        return ["Run python3 release_acceptance.py release/CodexCompanion-0.4.8-latest-dist.zip --skip-tests to see the exact syntax command failure."]
    if name == "release_zip_smoke":
        return ["Run python3 package_release.py 0.4.8, then rerun release_smoke_test.py on the latest dist zip."]
    if name == "install_dry_run":
        return ["Run python3 release_smoke_test.py release/CodexCompanion-0.4.8-latest-dist.zip --install-dry-run and fix the install/uninstall script failure."]
    ...
    if name == "release_sha256_manifest":
        ...
        return [
            "Run python3 package_release.py, then python3 build_pkg.py release/CodexCompanion-0.4.8-latest-dist.zip --json.",
    ...
    if name == "signed_pkg":
        return [
            ...
            "CLI alternative: python3 build_pkg.py release/CodexCompanion-0.4.8-latest-dist.zip --auto-sign --json.",
        ]
    if name == "notarized_pkg":
        return [
            ...
            "CLI alternative: python3 notarize_pkg.py release/CodexCompanion-0.4.8-latest.pkg --keychain-profile <profile> --json.",
        ]
