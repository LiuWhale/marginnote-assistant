...
- See the first Operation Compiler surface in Agent Workspace. `agent_plan` now exposes a structured `operationPlan`, `verificationPlan`, and compiler checks so write-capable actions show planned steps, write count, dry-run/confirmation requirements, and verification obligations before they are treated as real MarginNote edits.
