...
The current public release is **0.4.55**: https://github.com/LiuWhale/marginnote-assistant/releases/tag/v0.4.55. It keeps the no-timeout Codex CLI behavior and fixes first-click reply-to-mind-map generation for document-scoped MarginNote snapshots.
...
The active conversation is durably restored after the panel reloads. Each persisted session has a `session epoch`; delayed results or mutations whose epoch no longer matches are rejected, and a deleted session leaves a `tombstone` that rejects late callbacks instead of recreating the conversation. A failed queue item remains visible as retryable work rather than being acknowledged or discarded. A queued card or mind-map result is saved as a draft and always requires explicit confirmation before the native write bridge can run.

### Multi-File Source Workspace (`资料`)

Use the `资料 N` control in the chat header when one answer must read one or more explicitly selected files. The dedicated picker lists the current MarginNote document, resolvable notebook files, and uploaded files. Add local files through `Add Files`, inspect each read and text-extraction status, then click `验证资料` and `完成`. Use `全选可移除`, `取消全选`, and `移除所选` to shrink the current conversation membership without touching source files or upload records. An explicit set containing only one file still uses the managed workspace, Codex CLI, source acknowledgement, and write gate; only legacy conversations with no explicit source metadata retain the old single-document path.

Click `Add Files` to upload several local files in one picker operation. The native multiple picker accepts up to **20 files per operation**, each no larger than **20 MB**. Supported binary and text formats include PDF, DOCX, PPTX, Markdown and plain text, JSON/CSV/TSV, Jupyter notebooks, LaTeX/BibTeX/RST, common source-code/config/web formats, and PNG/JPEG/GIF/WebP/BMP/TIFF/HEIC/HEIF/AVIF/ICO images. The picker reports per-file upload progress and a final success/failure count; partial failures do not discard completed uploads. Successful files remain available, are automatically selected, and are then saved and validated as sources. Fix or reselect only failed files, and use repeated batches when more than 20 files are needed.
...
Use the source-management mode when a conversation needs a smaller set. `全选可移除`, `取消全选`, and `移除所选` select and remove only current conversation membership. They never delete originals, extracted/cached files, or upload-registry records. While `跟随当前文件` is enabled, the followed current document is protected from removal; disable follow-current before selecting it. A removal first updates and validates the new workspace. If either stage fails, Companion restores the previous membership and validates that restoration. If rollback fails, the panel shows an explicit warning and does not report the removal as complete.
