...
The 0.4.53 local HTTP action endpoint requires the install-scoped token stored at `~/.codex/marginnote-assistant/control/web-action-token`, an exact `127.0.0.1:48761` Host, and the installed file-origin CORS policy. The Web panel and `send_action.py` attach the token as a request header; it is never copied into action JSON, conversation history, or diagnostic logs. Request-provided document paths do not authorize workspace sources: candidates come only from Companion-owned PDF cache/index records, managed uploads, or server-owned MarginNote path resolution.
