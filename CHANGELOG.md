...
- Added a zero-message `codex.mn.objectIntake.v1` router in Notebook Workspace. It treats the current document, selection, or node as the workbench input object and routes it to Source Registry, Object Browser, Mindmap Studio, Card Factory, Workflow Builder, Skill Center, or Verification Center before the user sends a prompt.
- Extracted Knowledge Console Matrix construction into `knowledge_console.py` with dedicated `tests/test_knowledge_console.py`, keeping the zero-message notebook state matrix out of the Companion adapter layer.
- Extracted Object Intake construction into `object_intake.py` with dedicated `tests/test_object_intake.py`, keeping zero-message object routing out of the Companion adapter layer.
- Extracted Notebook Runbook construction into `notebook_runbook.py` with dedicated `tests/test_notebook_runbook.py`. It now owns `codex.mn.notebookRunbook.v1`, eight Runbook steps, continue-next-step actions, and safe AutoPlan construction, while Companion keeps only preflight storage and ledger integration.
- Added a first `codex.mn.objectTaskComposer.v1` in Notebook Workspace. It converts object routes into task drafts with evidence, expected output, route actions, optional `agent_plan` compile actions, and `codex.mn.objectTaskWorkflowCandidate.v1` workflow candidates that can start the existing Workflow Runtime without bypassing write confirmations.
- Extracted Object Task Composer construction into `object_task_composer.py` with dedicated `tests/test_object_task_composer.py`, keeping object route-to-task composition out of the Companion adapter layer.
- Added a first `codex.mn.workflowBuilderBoard.v1` in the workflow workspace. It groups task candidates, active runs, `waiting_confirmation` runs, and evidence into visible lanes through `workflowBuilderBoardPanel`, so long tasks are no longer represented only by pending counts or recent-run rows.
- Extracted Workflow Builder Board construction into `workflow_builder.py` with dedicated `tests/test_workflow_builder.py`, keeping the Companion service as an adapter instead of another home for v3 kernel logic.
- Added a zero-message `codex.mn.knowledgeConsoleMatrix.v1` in Notebook Workspace. It summarizes source inventory, native MN objects, mind-map baseline, card coverage, workflow runtime, Operation Ledger, and verification evidence before the user sends a prompt, with conservative `ready` / `action_required` / `blocked` / `waiting_evidence` states.
- Added a Source Registry transition to the Matrix source axis. Readable material now opens `open_source_registry`, while missing material still shows cache, file-picker, and path-management repair actions.
- Added an Object Browser transition to the Matrix MN-object axis. The axis requests `scan_mn_objects` while native scan evidence is missing, then switches to `open_object_browser` / `object_browser` after `native_object_scan` evidence exists.
- Added a Mindmap Studio transition to the Matrix mind-map baseline axis. The axis requests `mn_read_tree` while no tree cache exists, then switches to `open_mindmap_studio` after the current mind-map baseline is cached.
- Added Card Factory and Workflow Builder transitions to the Matrix. The card-coverage axis opens `open_card_factory`, and the Workflow axis switches from `workflow_list` to `open_workflow_builder` after a run exists.
- Expanded Notebook Runbook from six checks to eight checks by adding `source_inventory` and `card_coverage`. Auto prepare can now include source preflight before MN scan, mind-map baseline, and operation planning, while still avoiding automatic card generation or MarginNote writes.
- Linked the matrix verification axis directly to Verification Center. Its action now opens the operation workspace and refreshes `verification_report_list`, rather than sending users through Operation Ledger first.
- Added the first visible Verification Center in the operation workspace. It calls `verification_report_list`, aggregates current-object verification reports from Operation Ledger entries, Source Registry, and Skill Runtime evidence, and renders strict `PASS` / `FAIL` / `UNKNOWN` counts plus per-report rows.
- Added a first Verification Center repair action. Transaction reports blocked by `native_probe_missing` now expose `检查真实对象`, which submits the existing `request_mn_object_existence_probe` command to MN4 instead of leaving users to infer the next step.
- Added an in-panel Verification Center action-status line so repair actions show pending, submitted, and failed state without relying only on the chat transcript.
- Closed the first native-probe verification loop: `mnObjectExistenceProbeFinished` evidence is now normalized into transaction verification reports, so Verification Center can turn transaction `UNKNOWN` into `PASS` or `FAIL` after MN4 reports real object existence.
- Added the first object-level `codex.mn.verificationRepairPlan.v1`. Verification Center now groups non-PASS report actions into a repair plan, highlights the recommended action, and distinguishes `ready`, `action_required`, `waiting_evidence`, and `blocked` states.
- Added a primary `执行推荐修复` button to Verification Center repair plans. The button reads `repairPlan.recommendedAction`, disables itself when no safe action exists, and submits the same repair path as the detailed report action list.
- Added `ui_functional_acceptance.py`, a stdlib-only arbitrary-document UI functional acceptance gate. It verifies WebView static controls and behavior markers, Notebook Workspace kernels, workspace surface actions, Workflow Builder Board lanes, and no-scope native action guards against a synthetic document payload. With `--browser-render`, it launches a local headless browser and checks the rendered Agent Workspace DOM. With `--browser-interaction`, it drives real Chat/Workspace, every visible Workspace Navigator card, every Workbench tab, Command Pane, settings, and history clicks through the browser DevTools protocol. With `--browser-actions`, it stubs Companion responses, injects an arbitrary `MNObject` and notebook scope, clicks Agent Plan, MN object scan, mind-map tree read, Notebook Workspace, Runbook continue/auto-prepare, Object Browser refresh/filter, Object Graph, object relation save/cancel, Object Activity, Operation Ledger refresh/filter, Verification Center, recommended repair, Knowledge Search, mind-map target status, health, AI backend, logs, and history-scope buttons, and verifies the WebView sends the expected backend actions without showing a connection failure. The script is now required by release packaging, smoke tests, doctor, and release docs.
- Added `--browser-write-actions` to the arbitrary-document UI functional gate. It drives the reply "生成脑图树" button, draft save, AI edit accept/reject/review controls, Mindmap Studio Diff preview/apply, transaction verify/evidence/probe controls, and browser-preview native bridge recording for `write_draft`, `accept_ai_edit_transaction`, and `reject_ai_edit_transaction`.
- Added the settings-page `UI 功能验收` entry. The `uiFunctionalAcceptanceButton` calls `ui_functional_acceptance_summary`, runs the full arbitrary-document UI gate from inside the plugin, and renders PASS/BLOCK, passed checks, blockers, and detailed check output.
- The browser action gate now verifies the settings page actually renders `uiFunctionalAcceptanceLine` as `UI 功能验收：PASS / 11/11 / 阻塞 0` after clicking `uiFunctionalAcceptanceButton`, rather than only verifying the backend action was called.
- Expanded `webview_browser_actions` coverage to include `sendButton`, prompt clearing after send, `Enter key` chat submission, `conversation_new`, `conversationHistoryObjectButton`, `conversationHistoryAllButton`, context-scope switching, native `context` bridge refresh, file-path `settings_update`, `update_check`, download-page `open_url`, `diagnose_permissions`, `open_full_disk_access_settings`, `request_pdf_cache`, `request_native_capability_probe`, `notebook_runbook_preflight_record`, `object_graph`, `object_activity`, `object_graph_relation_save`, `verificationRepairPlanRecommendedButton`, `knowledge_index_search`, and `mindmap_target_status`. The report now includes `buttonActionDeltas`, so duplicate-action buttons such as refresh/filter and history-scope switches must each increment their own expected backend action count.
- Expanded `webview_browser_write_actions` to click the static `mindmapStudioVerifyButton` and `mindmapStudioRollbackButton` in addition to dynamic transaction controls, proving the Studio buttons reach `ai_edit_transaction_verify` and the reject bridge in a stubbed browser session.
- Added `webview_button_coverage` to the arbitrary-document UI gate. It enumerates every static WebView button and fails when a new button is not classified as actual browser-clicked, interaction-clicked, write-flow-clicked, panel-control, dynamic-runtime, form/destructive, file-picker, close-only, or hidden-runtime.

### Changed

- Notebook Workspace now disables native MN scan/tree-read actions when `topicid` is unavailable, instead of rendering buttons that can only fail with HTTP 400. Runbook, Knowledge Console Matrix, Object Intake, primary workspace actions, Object Browser scan, and mind-map tree read buttons now share the same no-scope guard.
- Updated the v3 documentation boundary so the current plugin no longer describes visual verification as entirely future work; the remaining v3 gap is a full verification studio with broader native probes, cross-workflow repair automation, and before/after object graph comparison.
- Added an explicit v3 product contract to the ultimate design and READMEs: v3 is accepted as a Notebook Knowledge IDE with object, operation, learning, evidence, skill, and automation contracts, not as the current chat/card/mind-map plugin with more buttons.
